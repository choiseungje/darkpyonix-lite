...
# ---------- SessionManager ----------
class StickySessionManager(SessionManager):
    """
    Session manager that determines kernel_id from file path during session creation.
    Enables kernel sharing across different users for the same file.
    """

    def __init__(self, *a, **kw):
        """Initialize the sticky session manager with logging setup."""
        super().__init__(*a, **kw)
        self.sticky_logger = logging.getLogger("StickySM")
        if not self.sticky_logger.handlers:
            h = logging.StreamHandler()
            h.setFormatter(logging.Formatter("[%(name)s] %(asctime)s - %(levelname)s - %(message)s"))
            self.sticky_logger.addHandler(h)
        self.sticky_logger.setLevel(logging.INFO)
        if getattr(self, "log", None):
            self.log.info("[StickySM] initialized (root_dir=%s)", getattr(self.parent, "root_dir", None))
        self.sticky_logger.info("initialized (root_dir=%s)", getattr(self.parent, "root_dir", None))
    ...
    def _debug(self, msg: str):
        ...
        self.sticky_logger.info(msg)
        if getattr(self, "log", None):
            self.log.info(f"[StickySM] {msg}")

    async def create_session(self, **kwargs):
        """
        Create session with deterministic kernel_id based on file path.
        This is async in jupyter_server 2.x.

        Args:
            **kwargs: Session creation parameters including model with path info

        Returns:
            Created session object
        """
        self._debug(
            "create_session kwargs.keys="
        ...
        kernel = model.get("kernel") or kwargs.get("kernel") or {}
        kernel_name = kernel.get("name") or kwargs.get("kernel_name") or "python3"

        # Normalize file path
        abs_path = _abs_norm(getattr(self.parent, "root_dir", None) if hasattr(self, "parent") else None, path)
        self._debug(f"create_session path='{path}' -> abs='{abs_path}', kernel_name='{kernel_name}'")

        if abs_path:
            # Generate deterministic ID based on file path only (user-agnostic)
            kid = _stable_kernel_id(abs_path, kernel_name, _namespace())
            self._debug(f"proposed shared kernel_id={kid} for all users")

            # If kernel doesn't exist, start it first; otherwise attach to existing
            km = self.kernel_manager  # type: ignore[attr-defined]
            if kid not in getattr(km, "_kernels", {}):
                self._debug(f"shared kernel {kid} not running; starting first")
                try:
                    # Handle both sync/async kernel managers
                    if hasattr(km, "start_kernel_async"):
                        await km.start_kernel_async(kernel_id=kid, kernel_name=kernel_name, path=abs_path)
                    else:
                        km.start_kernel(kernel_id=kid, kernel_name=kernel_name, path=abs_path)
                except Exception as e:
                    self._debug(f"start_kernel failed: {e}")
                    raise

            # Inject kernel.id into session request
            model.setdefault("kernel", {})["id"] = kid
            model.setdefault("kernel", {})["name"] = kernel_name
            model["path"] = abs_path
            kwargs["model"] = model
